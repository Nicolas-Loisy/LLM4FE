...
def test_length_operation():
    ...
    transform = TextProcessingTransform(new_column_name="length", columns_to_process=["text"], param={"operation": "length"})
...
def test_word_count_operation():
    ...
    transform = TextProcessingTransform(new_column_name="word_count", columns_to_process=["text"], param={"operation": "word_count"})
    assert transform.valid
    result = transform.transform(df)
...
def test_keyword_operation():
    ...
    transform = TextProcessingTransform(new_column_name="has_foo", columns_to_process=["text"], param={"operation": "keyword", "keyword": "foo"})
    assert transform.valid
    result = transform.transform(df)
...
def test_tfidf_operation():
    ...
    transform = TextProcessingTransform(new_column_name="tfidf", columns_to_process=["text"], param={"operation": "tfidf", "max_features": 2})
    assert transform.valid
    result = transform.transform(df)
...
def test_invalid_operation_invalid_flag():
    transform = TextProcessingTransform(new_column_name="invalid", columns_to_process=["text"], param={"operation": "invalid"})
